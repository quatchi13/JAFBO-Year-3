...
namespace JAFnetwork
{
    ...
    public class BasicAttackChar : NetCommand
    {
        ...
        public void Inverse() {
            NetworkParser.playerCharacters[1].transform.eulerAngles = new Vector3(-eulerX, -eulerY, -eulerZ);
            //player.GetComponent<Animator>().SetBool("Attack", true);
            NetworkParser.playerCharacters[0].GetComponent<HealthSystem>().Damage(NetworkParser.playerCharacters[1].GetComponent<Attacking>().currentDamage);
            Debug.Log("slap");
        }
    ...
    public class CharacterOrderCommand: ServerCommand
    {
        ...
        public void Execute()
        {
            NetworkParser.playerCharacters[0] = PC_Manager.MasterList[ind1];
            NetworkParser.playerCharacters[1] = PC_Manager.MasterList[ind2];
        }
